...
"""The :mod:`lakeshore370ac.transport` module implements the lowest level
abstraction layer of the library.
...
layers. Custom transports should subclass :class:`lakeshore370ac.Transport` and
implement the `__read__()` and `__write__()` methods.
...
from analogreadout.external.lakeshore370ac.misc import wrap_exception
...
class Transport(object):
    ...
    library. Transports are intended to be used as context managers.
...
class Socket(Transport):
    """A compatible adapter for pythons socket.socket class.

    :param address: The socket address a tuple of host string and port. E.g.
        ::

            from analogreadout.external.lakeshore370ac.signal_recovery import SR7230
            from analogreadout.external.lakeshore370ac.transport import Socket
    ...
    if LooseVersion(VISA_VERSION) < LooseVersion('1.5'):
        ...
        class Visa(Transport):
            ...
            def __init__(self, *args, **kw):
                super(Visa, self).__init__()
                rm = visa.ResourceManager()
                self._instrument = rm.open_resource(*args, **kw)
